...
class PaymentScreenV2 extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PaymentScreenV2> createState() => _PaymentScreenV2State();
}

class _PaymentScreenV2State extends ConsumerState<PaymentScreenV2> {
  bool isLoading = true;
  ...
  Future<void> _loadOrderDetails() async {
     try {
        ...
        // 1. Get Group Settings via OrderContext
        final ctx = await repo.getOrderContext(widget.groupKey);
        if (ctx == null) throw Exception("Order not found");

        final group = ctx.order;
        
        final int storedRate = group.serviceFeeRate?.toInt() ?? 0;
        final double initialDiscount = group.discountAmount ?? 0;

        if (!_isLoaded) {
           // 預設啟用 10%；若訂單已存過非零費率則沿用
           isServiceFeeEnabled = true;
           serviceFeeRate = storedRate > 0 ? storedRate : 10;
        ...
        if (group.taxSnapshot != null) {
          taxProfile = TaxProfile(
              id: 'temp', 
              shopId: '', 
              rate: (group.taxSnapshot!['rate'] as num?)?.toDouble() ?? 0.0,
              isTaxIncluded: group.taxSnapshot!['is_tax_included'] ?? true,
              updatedAt: DateTime.now()
          );
        } else {
          taxProfile = await repo.getTaxProfile();
        }

        // 2. Get Active Items
        _itemDetails = group.items.where((i) => i.status != OrderingConstants.orderStatusCancelled).map((i) => {
          'id': i.id,
        ...
        _tableNames = ctx.tableNames;
        _pax = ctx.peopleCount;
        _createdAt = group.createdAt ?? DateTime.now();
        
        _calculateTotals();

       // Fetch Order Rank (Async)
       try {
          final count = await repo.getOrderRank(widget.groupKey);
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      // 0.5 ~ 2. Checkout Execution
      bool savedToHubOffline = false;
      debugPrint('💳 Checkout path: isHubDevice=$isHubDevice, isHubAvailable=${HubClient().isHubAvailable}, hasHubIp=${HubClient().hasHubIpConfigured}');
      try {
        if (HubClient().isHubAvailable) {
           // Hub Mode Checkout Routing
           final result = await HubClient().post('/checkout', {
             'checkout': {
                'id': const Uuid().v4(),
           ...
           if (result == null) throw '無法連線到主機，請確認主機 App 已開啟';
           savedToHubOffline = true;
        } else if (isHubDevice) {
           // Hub 機台本身：直接寫本機 SQLite，再由 syncOfflineOrders 推 Supabase
           await LocalDbService().updatePendingOrderGroupStatus(widget.groupKey, 'completed');
           await LocalDbService().insertPendingCheckout({
             'id': const Uuid().v4(),
             'order_group_id': widget.groupKey,
             'payment_method': payments.map((p) => p['method']).toSet().join(','),
             'final_amount': _totalAmount,
           ...
           HubServer().onOrderChanged?.call();
           ref.read(orderingRepositoryProvider).syncOfflineOrders();
           savedToHubOffline = true;
        } else {
           // Standard Supabase Logic (Non-Hub)
           // 1. Insert Payments
      ...
      if (!savedToHubOffline && _taxProfile != null && _taxProfile!.rate == 5.0 && invoiceError == null) {
        try {
          final repo = ref.read(orderingRepositoryProvider);
          final updatedCtx = await repo.getOrderContext(widget.groupKey);
          if (updatedCtx == null) throw Exception("Order not found after checkout");
          final updatedGroup = updatedCtx.order;
          ...
          final orderForProof = OrderGroup(
             ...
             checkoutTime: updatedGroup.checkoutTime,
             ezpayInvoiceNumber: updatedGroup.ezpayInvoiceNumber,
             ezpayRandomNum: updatedGroup.ezpayRandomNum,
             ezpayQrLeft: updatedGroup.ezpayQrLeft,
             ezpayQrRight: updatedGroup.ezpayQrRight,
             finalAmount: updatedGroup.finalAmount,
             buyerUbn: updatedGroup.buyerUbn,
          );

          final isB2B = updatedGroup.buyerUbn != null && updatedGroup.buyerUbn.toString().length == 8;
          // Note: we might not have carrier_num in OrderGroup entity properly yet, check manually from controllers
          final carrierNum = carrierNumController.text.trim();
          final hasCarrier = carrierNum.isNotEmpty;
      ...
      final bool needsInvoice = !savedToHubOffline
          ...
          && (invoiceError != null);
      // Hub 離線且需要開發票（5%稅率），或列印失敗，或發票開立失敗
      final bool needsHubInvoiceLater = savedToHubOffline && (_taxProfile?.rate == 5.0);
      if (receiptPrintFailed || needsInvoice || needsHubInvoiceLater) {
        try {
