...
class OrderScreenV2 extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<OrderScreenV2> createState() => _OrderScreenV2State();
}

class _OrderScreenV2State extends ConsumerState<OrderScreenV2> {
...
class _CartPanelState extends ConsumerState<_CartPanel> {
  ...
  Future<void> _loadSubmittedItems() async {
    ...
    try {
      final items = await ref.read(orderingRepositoryProvider).getOrderItems(widget.orderGroupId!);
      if (mounted) {
        setState(() {
          _submittedItems = items.map((i) => <String, dynamic>{
            'id': i.id,
  ...
  /// Marks item as cancelled in UI immediately and fires async void API.
  void _voidSubmittedItemById(String itemId) {
    final idx = _submittedItems.indexWhere((i) => i['id'] == itemId);
    if (idx < 0) return;
    final item = _submittedItems[idx];
    if (item['status'] == OrderingConstants.orderStatusCancelled) return;
    setState(() {
      _submittedItems[idx] = <String, dynamic>{...item, 'status': OrderingConstants.orderStatusCancelled};
      _selectedReprintIds.remove(itemId);
    });
    _doVoidSubmittedItem(item, idx);
  }

  Future<void> _doVoidSubmittedItem(Map<String, dynamic> item, int idx) async {
    try {
      final user = ref.read(authStateProvider).value;
      ...
      if (mounted) {
        if (idx < _submittedItems.length) setState(() => _submittedItems[idx] = item);
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if (index < submittedCount) {
                              ...
                              final swipeDetector = GestureDetector(
                                behavior: HitTestBehavior.translucent,
                                onHorizontalDragUpdate: isCancelled ? null : (details) {
                                  final next = (swipeOffset + details.delta.dx).clamp(-300.0, 0.0);
                                  setState(() => _swipeOffsets[itemId] = next);
                                  if (next <= -300.0) {
                                    _swipeOffsets.remove(itemId);
                                    _voidSubmittedItemById(itemId);
                                  }
                                },
                                onHorizontalDragEnd: isCancelled ? null : (details) {
                                  if (swipeOffset <= -150 || details.velocity.pixelsPerSecond.dx < -800) {
                                    _swipeOffsets.remove(itemId);
                                    _voidSubmittedItemById(itemId);
                                  } else {
                                    setState(() => _swipeOffsets.remove(itemId));
                                  }
                                },
...
class _TableOrderHistorySheetState extends ConsumerState<_TableOrderHistorySheet> {
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          if (ctx != null) {
            orderItems = ctx.order.items.map((i) => <String, dynamic>{
              'id': i.id,
              'item_id': i.menuItemId,
              'item_name': i.itemName,
              'quantity': i.quantity,
  ...
  void _voidItemById(String itemId) {
    final idx = orderItems.indexWhere((i) => i['id'] == itemId);
    if (idx < 0) return;
    final item = orderItems[idx];
    if (item['status'] == OrderingConstants.orderStatusCancelled) return;
    setState(() => orderItems[idx] = <String, dynamic>{...item, 'status': OrderingConstants.orderStatusCancelled});
    _doVoidItem(item, idx);
  }

  Future<void> _doVoidItem(Map<String, dynamic> item, int idx) async {
    try {
      final repository = ref.read(orderingRepositoryProvider);
      final user = ref.read(authStateProvider).value;
      final staffName = (user?.name != null && user!.name.trim().isNotEmpty) ? user.name : (user?.email ?? '');
      ...
      if (mounted && idx < orderItems.length) setState(() => orderItems[idx] = item);
    }
  }
                    ...
                    : orderItems.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (ctx, index) {
                              ...
                              final itemId2 = item['id'] as String;
                              final swipeOffset2 = _swipeOffsets[itemId2] ?? 0.0;

                              return KeyedSubtree(
                                key: ValueKey('history_item_$itemId2'),
                                child: GestureDetector(
                                  behavior: HitTestBehavior.translucent,
                                  onHorizontalDragUpdate: isCancelled ? null : (details) {
                                    final next = (swipeOffset2 + details.delta.dx).clamp(-300.0, 0.0);
                                    setState(() => _swipeOffsets[itemId2] = next);
                                    if (next <= -300.0) {
                                      _swipeOffsets.remove(itemId2);
                                      _voidItemById(itemId2);
                                    }
                                  },
                                  onHorizontalDragEnd: isCancelled ? null : (details) {
                                    if (swipeOffset2 <= -150 || details.velocity.pixelsPerSecond.dx < -800) {
                                      _swipeOffsets.remove(itemId2);
                                      _voidItemById(itemId2);
                                    } else {
                                      setState(() => _swipeOffsets.remove(itemId2));
                                    }
                                  },
                                  child: Transform.translate(
                                    offset: Offset(swipeOffset2, 0),
                                    child: content,
                                  ),
                                ),
                              );
