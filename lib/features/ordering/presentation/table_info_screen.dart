...
class _TableInfoScreenState extends ConsumerState<TableInfoScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final ctx = await repo.getOrderContext(widget.orderGroupId);

      if (mounted && ctx != null) {
        final List<Map<String, dynamic>> rawItemsList = ctx.order.items.map((i) => {
          'id': i.id,
        ...
        String getVisualIdentity(Map<String, dynamic> item) {
          ...
          final String note = (item['note'] ?? '').toString().replaceAll(RegExp(r'\| 刪除:.*'), '').trim();
          final List<dynamic> mods = item['modifiers'] ?? [];
        ...
        setState(() {
          orderGroupData = {
            'created_at': ctx.order.createdAt?.toIso8601String() ?? '',
            'pax': ctx.peopleCount,
            'pax_adult': ctx.order.paxAdult,
            'pax_child': ctx.order.paxChild,
            'note': '',
            'staff_name': ctx.order.staffName,
          };
  ...
  Future<void> _deleteItem(int index) async {
    ...
    setState(() {
      orderItems[index]['status'] = OrderingConstants.orderStatusCancelled;
  ...
  Future<void> _processReprintItem(int index) async {
    ...
    try {
      ...
      await ref.read(orderingRepositoryProvider).reprintSingleItem(
          ...
          item: orderItem,
          tableName: widget.tableName,
          staffName: orderGroupData?['staff_name']);
  ...
  Future<void> _undoDeleteItem(int index, List<String> itemIds, double itemTotal) async {
    ...
    try {
      await ref.read(orderingRepositoryProvider).undoVoidOrderItem(widget.orderGroupId, itemIds);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle contentStyle = TextStyle(color: Theme.of(context).colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.bold);
              ...
              children: [
                Row(
                  children: [
                    IconButton(
                      icon: Icon(
                        widget.embedded ? CupertinoIcons.xmark : CupertinoIcons.back,
            ...
            child: isLoading
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      if (mods != null && mods is List) {
                        for (var m in mods) {
                          if (m is Map) {
                            ...
                            if (modPrice > 0) modName += " (+\$${modPrice.toInt()})";
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (isCancelled) Text("(已刪除)", style: TextStyle(color: Theme.of(context).colorScheme.error, fontSize: 12)),
                                ],
                              ),
                            ),
                            SizedBox(width: 50, child: Text("x$qty", style: isCancelled ? contentStyle.copyWith(color: Theme.of(context).disabledColor) : contentStyle, textAlign: TextAlign.center)),
                            SizedBox(width: 80, child: Text("\$${lineTotal.toStringAsFixed(0)}", style: isCancelled ? contentStyle.copyWith(color: Theme.of(context).disabledColor, decoration: TextDecoration.lineThrough) : contentStyle, textAlign: TextAlign.right)),
                            if (!isCancelled) ...[
                              const SizedBox(width: 8),
                              IconButton(
                      ...
                      if (isCancelled) return rowContent;
  ...
  Widget _buildHeaderInfo(Map<String, dynamic> data) {
    ...
    final String paxStr = "${data['pax'] ?? 0} 人";
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Row(
              children: [
                _InfoChip(icon: CupertinoIcons.person_2_fill, label: paxStr),
                const SizedBox(width: 8),
                GestureDetector(
                  onTap: _showUpdatePaxDialog,
                  child: Container(
                    ...
                    child: Text("修改", style: TextStyle(color: Theme.of(context).colorScheme.primary, fontSize: 12)),
                  ),
                ),
              ],
            ),
  ...
  Future<void> _showUpdatePaxDialog() async {
    final repo = ref.read(orderingRepositoryProvider);
    final contextObj = await repo.getOrderContext(widget.orderGroupId);
    if (contextObj == null) return;

    int adult = contextObj.order.paxAdult;
    int child = contextObj.order.paxChild;

    // Fallback if paxAdult is 0 but peopleCount > 0
    if (adult == 0 && child == 0 && contextObj.peopleCount > 0) {
      adult = contextObj.peopleCount;
    }
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setStateDialog) {
            return AlertDialog(
              backgroundColor: Theme.of(context).cardColor,
              title: const Text("調整人數"),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  _buildPaxAdjustRow("成人", adult, (val) => setStateDialog(() => adult = val)),
                  const SizedBox(height: 16),
                  _buildPaxAdjustRow("小孩", child, (val) => setStateDialog(() => child = val)),
                ],
              ),
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    try {
                      final repo = ref.read(orderingRepositoryProvider);
                      await repo.updateOrderGroupPax(
                        widget.orderGroupId,
                        adult + child,
                        adult: adult,
                        child: child,
  ...
  Widget _buildPaxAdjustRow(String label, int value, Function(int) onChanged) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label),
        Row(
          children: [
            IconButton(onPressed: value > 0 ? () => onChanged(value - 1) : null, icon: const Icon(CupertinoIcons.minus_circle)),
            SizedBox(width: 30, child: Text(value.toString(), textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
            IconButton(onPressed: () => onChanged(value + 1), icon: const Icon(CupertinoIcons.plus_circle)),
          ],
        ),
      ],
    );
...
class _SwipeToDeleteRow extends StatefulWidget {
  ...
  const _SwipeToDeleteRow({super.key, required this.child, required this.onDelete});
  @override
...
class _SwipeToDeleteRowState extends State<_SwipeToDeleteRow> with SingleTickerProviderStateMixin {
  ...
  Future<void> _animateTo(double target) {
    ...
    _controller.reset();
    final animation = Tween<double>(begin: start, end: target).animate(CurvedAnimation(parent: _controller, curve: Curves.easeOut));
      ...
      children: [
        Positioned.fill(
          child: GestureDetector(
            onTap: () => _animateTo(-MediaQuery.of(context).size.width).then((_) => widget.onDelete()),
            child: Container(
